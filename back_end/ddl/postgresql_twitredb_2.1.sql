...
COPY public.twitter_api_tbl (id, id_str, screen_name, created_at, create_time, text, trend, user_id, user_id_str, use_name, sys_create_date, hidden_flag, delete_flag) FROM stdin;
\.
...
COPY public.twitter_sysid_tbl (sys_id, created_at, as_of, delete_flag) FROM stdin;
\.
...
COPY public.twitter_trends_tbl (sys_id, name, tweet_volume, query, delete_flag) FROM stdin;
\.
...
REVOKE ALL ON SCHEMA public FROM fujinotakamasa;
